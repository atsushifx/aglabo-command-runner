...
describe('Given: AGTCommandErrorType enum is defined', () => {
  ...
  describe('When: Creating command-result.types.ts with AGTCommandErrorType', () => {
    ...
    it('Then: [正常] - public enum created', () => {
      // Arrange: Get the enum
      const errorTypeEnum = AGTCommandErrorType;

      // Act: Check if enum exists
      const isDefined = errorTypeEnum !== undefined;

      // Assert: Verify enum is defined
      expect(isDefined).toBe(true);
      expect(typeof errorTypeEnum).toBe('object');
    });
  });
  ...
  describe('When: Checking invalid values', () => {
    ...
    it.each<string>([
      ...
      (invalidKey) => {
        ...
        expect((AGTCommandErrorType as any)[invalidKey]).toBeUndefined();
      },
    );
  });
  ...
  describe('When: Checking type accessibility and export', () => {
    ...
    it('Then: [正常] - enum is properly exported and accessible as AGTCommandErrorType', () => {
      ...
      // Act: Verify the type can be used
      const isAccessible = errorType === AGTCommandErrorType.TypeErrors;

      // Assert: Verify type is accessible
