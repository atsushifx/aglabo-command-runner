...
BDD 実装は以下の 9 ステップで構成されます。各ステップは順序通り実行し、スキップ不可。
...
Step 3: deckrd-bdd-coder で単一タスク実装
    ↓
Step 4: 実装したタスクをチェック (新規)
    ├─ tasks.md でタスクを [x] にマーク
    └─ TodoWrite で進捗同期
    ↓
Step 5: 繰り返し判定
    ├─ YES: Step 3 へ戻る (次のタスク)
    └─ NO: Step 6 へ進む
    ↓
Step 6: 状態確定と進捗記録
    ↓
Step 7: 品質ゲート実行 (Lint + 型チェック)
    ↓
Step 8: Refactor フェーズ
    ↓
Step 9: 完了判定
```
...
1. ステップ順序の厳密性: 1→2→3→4→5→6→7→8→9 の順序でスキップ不可
2. 単一タスク原則: deckrd-bdd-coder は 1 タスク専用 (並行実行禁止)
3. 最小実装の遵守: Green フェーズは過剰実装禁止
4. 即座の記録: ステップ 4 で実装したタスクを即座にマーク（進捗の可視化）
5. 品質ゲート必須: ステップ 7 実施は必須 (3回以上失敗時、ユーザーに相談)
6. Refactor の不可欠性: ステップ 8 でコード品質を統合的に改善
7. 完了条件の明確性: ステップ 9 のすべてのチェック項目を満たす必要あり
...
- deckrd-bdd-coder の処理完了条件: **全テスト合格**
